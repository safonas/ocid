...
Download standalone binaries or tarballs for Linux (`x86_64`, `arm64`),
Android arm64 (Termux) and
...
Android arm64 devices are `aarch64` (`arm64-v8a`), but the `linux-arm64`
assets are glibc and will not run under Termux (bionic libc). Use the
`ocid-android-arm64.tar.gz` release asset instead — no `.deb`, no systemd:

```sh
uname -m   # -> aarch64
pkg install proot tar
curl -sSfL https://github.com/safonas/ocid/releases/latest/download/ocid-android-arm64.tar.gz | tar -xzf -
mkdir -p $PREFIX/bin && mv ocid ocictl ocitop $PREFIX/bin/
chmod +x $PREFIX/bin/ocid $PREFIX/bin/ocictl $PREFIX/bin/ocitop
OCID_HOME=~/.ocid ocid &
ocictl status
```

To build it yourself: `just pkg-android` (needs podman; downloads the pinned
Android NDK once into a cache volume).
